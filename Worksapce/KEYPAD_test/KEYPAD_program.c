/*
 *  file Name : KEYPAD_program.c
 *  Author    : Khaled Ahmed Elwan
 *  Date : 27/8/2026
 *  Version : 1.0
 */


#include "STD_TYPES.h"
#include "BIT_MATH.h"
#include "DIO_interface.h"
#include "KEYPAD_interface.h"
#include "KEYPAD_private.h"
#include "KEYPAD_config.h"


#include "util/delay.h"

void KPD_Init(void)
{
	/* Make the 4 rows pulled-up */
	/* Drive them as inputs */
	DIO_SetPinDirection(KPD_PORT, KPD_R0, DIO_INPUT);
	DIO_SetPinDirection(KPD_PORT, KPD_R1, DIO_INPUT);
	DIO_SetPinDirection(KPD_PORT, KPD_R2, DIO_INPUT);
	DIO_SetPinDirection(KPD_PORT, KPD_R3, DIO_INPUT);

	/* Set them high */
	DIO_SetPinValue(KPD_PORT, KPD_R0, DIO_HIGH);
	DIO_SetPinValue(KPD_PORT, KPD_R1, DIO_HIGH);
	DIO_SetPinValue(KPD_PORT, KPD_R2, DIO_HIGH);
	DIO_SetPinValue(KPD_PORT, KPD_R3, DIO_HIGH);


	/* Make the 4 columns outputs */
	DIO_SetPinDirection( KPD_PORT , KPD_C0 , DIO_OUTPUT );
	DIO_SetPinDirection( KPD_PORT , KPD_C1 , DIO_OUTPUT );
	DIO_SetPinDirection( KPD_PORT , KPD_C2 , DIO_OUTPUT );
	DIO_SetPinDirection( KPD_PORT , KPD_C3 , DIO_OUTPUT );

	/* Initiate them high at starting */
	DIO_SetPinValue    ( KPD_PORT , KPD_C0 , DIO_HIGH );
	DIO_SetPinValue    ( KPD_PORT , KPD_C1 , DIO_HIGH );
	DIO_SetPinValue    ( KPD_PORT , KPD_C2 , DIO_HIGH );
	DIO_SetPinValue    ( KPD_PORT , KPD_C3 , DIO_HIGH );

}


u8 KPD_u8GetPressed(void){
	u8 ReturnData = NOTPRESSED;
	u8 GetPressed;

	/* Iterators */
	u8 Row;
	u8 Col;

	for(Col = 0 + KPD_COL_INIT; Col < KPD_COL_END + 1; Col++)
	{
		DIO_SetPinValue(KPD_PORT, Col, DIO_LOW); // low on current column

		/* check rows for current columns */
		for(Row = 0 + KPD_ROW_INIT; Row < KPD_ROW_END + 1; Row++)
		{
			// check the current row: get its value (in GetPressed variable)
			DIO_GetPinValue(KPD_PORT, Row, &GetPressed);
			/* GetPressed = DIO_GetPinValue(KPD_PORT, Row); */

			if(GetPressed == 0)
			{
				//_delay_ms(10);
				DIO_GetPinValue(KPD_PORT, Row, &GetPressed);	// again
				if(GetPressed == 0)
					/* Get the input pressed */
					ReturnData = KPD_u8Buttons[2 - KPD_ROW_INIT][6 - KPD_COL_INIT];
			}

		}

		// return the column high
		DIO_SetPinValue(KPD_PORT, Col, DIO_HIGH);

	}
	return ReturnData;
}
