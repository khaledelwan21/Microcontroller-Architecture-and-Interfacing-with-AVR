/*
 *  file Name : Stepper_Motor_Program.c
 *  Created on: 30/8/2026
 *  Author    : Khaled Ahmed Elwan
 */

#include "util/delay.h"

#define F_CPU 8000000UL


#include "Stepper_Motor_Interface.h"
#include "Stepper_Motor_Config.h"
#include "DIO_interface.h"

//----------------------------------------------------------------------------------------------------------------------------------------------------
/*
* Breif : This Function initialize the pins which connected to stepper motor as output pins
* Parameters : Nothing
* return : Nothing
*/
void STEPPER_voidInit( void ){

	DIO_SetPinDirection( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_OUTPUT );
	DIO_SetPinDirection( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_OUTPUT );
	DIO_SetPinDirection( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_OUTPUT );
	DIO_SetPinDirection( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_OUTPUT );

	DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW );
	DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW );
	DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW );
	DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW );

}

//----------------------------------------------------------------------------------------------------------------------------------------------------
/*
* breif : This Function used to  stepper motor on
* Parameters :
            => Type : (STEPPER_FULL_STEP or STEPPER_HALF_STEP)
			=> Direction (STEPPER_CLOCK_WISE or STEPPER_ANTI_CLOCK_WISE)
			=> Speed
			=> Degree
* return : void
*/
void STEPPER_voidOn  ( u8 Copy_u8StepType , u8 Copy_u8Direction , u8 Copy_u8Speed , u16 Copy_u16Degree ){

	u32 LOC_u16Iterator = 0 ;

	if( Copy_u8StepType == STEPPER_FULL_STEP ){

		if( Copy_u8Direction == STEPPER_CLOCK_WISE ){

			for( LOC_u16Iterator = 0 ; LOC_u16Iterator < ( ( (u32)Copy_u16Degree * 2048 ) / 360 ) /  4; LOC_u16Iterator++  ){

				//Speed Variation Ratio = 1/64 = 0.015625
				//Stride Angle = 5.625° / 64 = 0.0875°


				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_HIGH );
				_delay_ms( Copy_u8Speed );


			}

		}else if( Copy_u8Direction == STEPPER_ANTI_CLOCK_WISE ){

			for( LOC_u16Iterator = 0 ; LOC_u16Iterator < ( ( (u32)Copy_u16Degree * 256 ) / 45 ) / 4 ; LOC_u16Iterator++  ){

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_HIGH );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );


			}

		}

	}else if( Copy_u8StepType == STEPPER_HALF_STEP ){

		if( Copy_u8Direction == STEPPER_CLOCK_WISE ){

			for( LOC_u16Iterator = 0 ; LOC_u16Iterator < ( ( (u32)Copy_u16Degree * 512 ) / 45 ) / 8 ; LOC_u16Iterator++  ){

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_HIGH );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_HIGH );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_HIGH );
				_delay_ms( Copy_u8Speed );



			}

		}else if( Copy_u8Direction == STEPPER_ANTI_CLOCK_WISE ){


			for( LOC_u16Iterator = 0 ; LOC_u16Iterator < ( ( (u32)Copy_u16Degree * 512 ) / 45 ) / 8 ; LOC_u16Iterator++  ){
				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_HIGH );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_HIGH );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_HIGH );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

				DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_HIGH );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW  );
				DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW  );
				_delay_ms( Copy_u8Speed );

			}

		}

	}

}

/*
* Breif : This Function This Function used to  stepper motor off
* Parameters : Nothing
* return : Nothing
*/
void STEPPER_voidOff ( void ){

	DIO_SetPinValue( STEPPER_PORT , STEPPER_BLUE_PIN   , DIO_LOW );
	DIO_SetPinValue( STEPPER_PORT , STEPPER_PINK_PIN   , DIO_LOW );
	DIO_SetPinValue( STEPPER_PORT , STEPPER_YELLOW_PIN , DIO_LOW );
	DIO_SetPinValue( STEPPER_PORT , STEPPER_ORANGE_PIN , DIO_LOW );

}
